...
const port = parsePort(process.env.MCP_HTTP_PORT, 3703);
...
app.post("/mcp", async (req: any, res: any) => {
  ...
  try {
    ...
    console.error("taiko-bridge http error:", error);
  }
});
...
app.listen(port, host, () => {
  console.log(`taiko-bridge HTTP MCP (read-only) listening on http://${host}:${port}/mcp`);
});
