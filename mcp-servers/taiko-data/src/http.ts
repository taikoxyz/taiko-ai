...
const port = parsePort(process.env.MCP_HTTP_PORT, 3701);
...
app.post("/mcp", async (req: any, res: any) => {
  ...
  try {
    ...
    console.error("taiko-data http error:", error);
  }
});
...
app.listen(port, host, () => {
  console.log(`taiko-data HTTP MCP listening on http://${host}:${port}/mcp`);
});
