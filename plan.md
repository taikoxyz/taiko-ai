...
**On-chain data query MCP for Taiko Mainnet + Hoodi**

- **Base**: Fork `crazyrabbitLTC/mcp-ethers-server` (MIT, TypeScript, 40+ tools)
- **Changes**: Swap RPC/API URLs to Taiko; add 5 Taiko-specific tools
- **Tools (12 total)**: get_balance, get_transactions, get_block_info, get_transaction_info, get_contract_abi, read_contract, get_token_transfers, get_gas_price, get_block_number, + `get_l1_checkpoint`, `get_bridge_message_status`, `get_anchor_block_state`, `get_nft_holdings`, `search`
...
**Node health monitoring MCP for simple-taiko-node**

- **Base**: Build from scratch with **Python FastMCP** + `docker` SDK
...
- **Why Python**: Docker SDK is superior to `dockerode` for log streaming and container management
