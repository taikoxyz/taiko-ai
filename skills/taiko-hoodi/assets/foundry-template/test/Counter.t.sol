...
///      Fork test: FOUNDRY_PROFILE=layer2 forge test --fork-url https://rpc.hoodi.taiko.xyz
...
/// @dev Run with: FOUNDRY_PROFILE=layer2 forge test --fork-url https://rpc.hoodi.taiko.xyz
contract TaikoForkTest is Test {
    function test_BridgeExists() public view {
        // Skip if not running on fork
        if (block.chainid != TaikoHoodiAddresses.TAIKO_HOODI_CHAIN_ID) {
            return;
        }

        // Verify bridge contract has code
        address bridge = TaikoHoodiAddresses.L2_BRIDGE;
        uint256 codeSize;
        assembly {
    ...
    function test_SignalServiceExists() public view {
        ...
        address signalService = TaikoHoodiAddresses.L2_SIGNAL_SERVICE;
        uint256 codeSize;
        assembly {
