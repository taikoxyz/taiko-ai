...
Requires: git `^2`, Go `^1.21`, make `^4`.

## Build

```bash
# taiko-geth (execution engine)
git clone https://github.com/taikoxyz/taiko-geth.git && cd taiko-geth
git checkout <release-tag>   # https://github.com/taikoxyz/taiko-geth/releases
make geth

# taiko-client (consensus client)
git clone https://github.com/taikoxyz/taiko-mono.git
cd taiko-mono/packages/taiko-client
git checkout <release-tag>   # https://github.com/taikoxyz/taiko-mono/releases
make build
```

Check release notes — some are network-specific.
...
```bash
openssl rand -hex 32 > jwt.txt   # Copy to both taiko-geth and taiko-client dirs
```
...
./build/bin/geth --taiko --networkid 167000 --gcmode archive \
  ...
  --authrpc.addr "0.0.0.0" --authrpc.port 28551 --authrpc.vhosts "*" --authrpc.jwtsecret ./jwt.txt \
  --http --http.api admin,debug,eth,net,web3,txpool,miner,taiko --http.addr "0.0.0.0" --http.port 28545 --http.vhosts "*" \
  --ws --ws.api admin,debug,eth,net,web3,txpool,miner,taiko --ws.addr "0.0.0.0" --ws.port 28546 --ws.origins "*" \
  --gpo.defaultprice "10000000" --port 30304 --syncmode full --state.scheme=path
```

**Hoodi testnet** (`--networkid 167013`):
```bash
./build/bin/geth --taiko --networkid 167013 --gcmode archive \
  --datadir ./data/taiko-geth --metrics --metrics.expensive --metrics.addr "0.0.0.0" \
  ...
  --http --http.api admin,debug,eth,net,web3,txpool,miner,taiko --http.addr "0.0.0.0" --http.port 28545 --http.vhosts "*" \
  --ws --ws.api admin,debug,eth,net,web3,txpool,miner,taiko --ws.addr "0.0.0.0" --ws.port 28546 --ws.origins "*" \
  --gpo.ignoreprice "100000000" --port 30304 --syncmode full --state.scheme=path
```
...
./bin/taiko-client driver \
  ...
  --taikoInbox 0x06a9Ab27c7e2255df1815E6CC0168d7755Feb19a --taikoAnchor 0x1670000000000000000000000000000000010001 \
  --preconfirmation.whitelist 0xFD019460881e6EeC632258222393d5821029b2ac --preconfirmation.serverPort 9871 \
  --jwtSecret ./jwt.txt --p2p.sync --p2p.checkPointSyncUrl https://rpc.mainnet.taiko.xyz \
  ...
  --p2p.listen.ip 0.0.0.0 --p2p.useragent taiko --p2p.listen.tcp 4001 --p2p.advertise.tcp 4001 \
  --p2p.listen.udp 30303 --p2p.advertise.tcp 30303 --p2p.peerstore.path /node-keys/peerstore \
  --p2p.discovery.path /node-keys/discv5 --p2p.advertise.ip ${YOUR_EXTERNAL_IP} --p2p.priv.raw ${PRIVATE_KEY}
```
...
./bin/taiko-client driver \
  --l1.ws ${L1_WS} --l2.ws ws://localhost:28546 --l1.beacon ${L1_BEACON_URL} --l2.auth http://localhost:28551 \
  --taikoInbox 0xf6eA848c7d7aC83de84db45Ae28EAbf377fe0eF9 --taikoAnchor 0x1670130000000000000000000000000000010001 \
  --preconfirmation.whitelist 0x83AE31678B9f255346Af4636B4726A84c3bD2886 --preconfirmation.serverPort 9871 \
  --jwtSecret ./jwt.txt --p2p.sync --p2p.checkPointSyncUrl https://rpc.hoodi.taiko.xyz \
...
**Without P2P:** Remove all `--p2p.*` and `--preconfirmation.*` flags except `--p2p.sync` and `--p2p.checkPointSyncUrl`. Chain head will lag behind preconfirmed blocks.

## P2P Notes

- `YOUR_EXTERNAL_IP`: Must be publicly reachable at TCP 4001 + UDP 30303. Find with `curl ifconfig.me`. Or use `--p2p.nat` instead (auto NAT, not recommended for production).
- `PRIVATE_KEY`: 32-byte hex for peer ID — no funds needed. Alternative: `--p2p.priv.path <file>`.
